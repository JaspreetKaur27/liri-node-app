...
function ShowTweets()
{
    var params = {screen_name: 'Jaspreet'};
    client.get('statuses/user_timeline', params, function(error, tweets, response){
        if(!error)
        {
            for(var i = 0; i < tweets.length; i++)
            {
                ...
                console.log(`@JaspreetKaur: ${tweets[i].text} Created At: ${date.substring(0,19)}`);
                console.log(" ");

                
                fs.appendFile("log.txt", "@JaspreetKaur: " +tweets[i].text +  "Created At: " + date.substring(0,19) + "\n");
